...
//        printf("Example: testfloat_compress_ts sz.config QCLOUDf /home/sdi/Data/Hurricane-ISA/consecutive-steps 48 500 500 100\n");
    ...
    char cmprFilePath[640];
    for (i = 0; i < timesteps; i++) {
//        printf("simulation time step %d\n", i);
        size_t outsize;
        sprintf(cmprFilePath, "/tmp/sztime/%05d.sz2", i);
        unsigned char *byte = readByteData(cmprFilePath, &outsize, &status);
        totalOutSize+=outsize;
        cost_start();
        SZ_decompress_ts(byte, outsize);
        cost_end();
...
//        printf("Decompress_time = %f\n", totalCost);
        ...
        printf("%d diff=%.6f, psnr=%.3f, nsmse=%e\n\n", i, max_diff, psnr, nrmse);
